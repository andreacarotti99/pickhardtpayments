...
class ReplicateBestStrategy:
    ...
    def run(self):
        ...
        print("liquidity")
        print(simulation_1.oracle_lightning_network.get_liquidity(highest_ratio_node, self._channel_graph.get_connected_nodes(highest_ratio_node)[0]))
        ...
        #print(simulation_2.oracle_lightning_network.get_liquidity(highest_ratio_node, self._channel_graph.get_connected_nodes(highest_ratio_node)[0]))

        self.remove_and_connect(highest_ratio_node, simulation_1.oracle_lightning_network)
    ...
    def remove_and_connect(self, node, oracle: OracleLightningNetwork):
        ...
        for i in range(len(neighbors)):
            ...
            for j in range(i+1, len(neighbors)):
                ...
                new_liquidity_i = liquidity_to_divide_i / (len(list(self._channel_graph.network.predecessors(node))) - 1)

                liquidity_to_divide_j = oracle.get_liquidity(neighbors[j], node)
                new_liquidity_j = liquidity_to_divide_j / (len(list(self._channel_graph.network.predecessors(node))) - 1)
                ...
                oracle_channel = OracleChannel(self._channel_graph.get_channel_without_short_channel_id(neighbors[i], neighbors[j]), new_liquidity_i)
                ...
                oracle_channel_rev = OracleChannel(self._channel_graph.get_channel_without_short_channel_id(neighbors[j], neighbors[i]), new_liquidity_j)
